...
coding_list <- list(Q17_1 = c("1" = "Male", "2" = "Female", "3" = "Non-Binary", "4" = "Non Conforming", "5" = "Queer Identified"))
...
# Recode gender according to coding_list
psp_data$Q17_1 <- factor(psp_data$Q17_1, levels = names(coding_list$Q17_1), labels = coding_list$Q17_1)

# Overall Age Range
age_range <- range(psp_data$Age, na.rm = TRUE)

# Overall Mean and SD for Age
age_mean <- round(mean(psp_data$Age, na.rm = TRUE), 3)
age_sd <- round(sd(psp_data$Age, na.rm = TRUE), 3)

# Mean and SD by Gender
age_stats_by_gender <- psp_data %>%
  group_by(Q17_1) %>%
  summarise(
    age_mean = round(mean(Age, na.rm = TRUE), 3),
    age_sd = round(sd(Age, na.rm = TRUE), 3)
  )

# Print Results
print(paste("Overall Age Range:", age_range[1], "to", age_range[2]))
print(paste("Overall Age Mean:", age_mean))
print(paste("Overall Age SD:", age_sd))
print("Age Stats by Gender:")
...
measurement_fit_ASI <- sem(measurement_model_ASI, data=psp_data, estimator = "MLR")
...
# For ASI
fit_stats_ASI <- fitMeasures(measurement_fit_ASI, c("cfi", "tli", "rmsea", "srmr", "chisq", "df", "pvalue"))
print("Fit Statistics for ASI Measurement Model (refined):")
print(fit_stats_ASI)
summary(measurement_fit_ASI)

# For AMI
fit_stats_AMI <- fitMeasures(measurement_fit_AMI, c("cfi", "tli", "rmsea", "srmr", "chisq", "df", "pvalue"))
...
#Get a Single Refined List

# Function to combine two lists and remove duplicates
combine_and_remove_duplicates <- function(list1, list2) {
  # Combine the lists into a new list
  combined_refined_scales_list <- modifyList(list1, list2)
  
  # Remove duplicate elements from each sublist in the combined list
  for (latent_variable in names(combined_refined_scales_list)) {
    if (is.list(combined_refined_scales_list[[latent_variable]])) {
      combined_refined_scales_list[[latent_variable]] <- unique(unlist(combined_refined_scales_list[[latent_variable]]))
    }
  }
  return(combined_refined_scales_list)
}

# Get the third list that combines the two filtered lists without duplicates
combined_refined_scales_list <- combine_and_remove_duplicates(refined_scales_list_filtered_ASI, refined_scales_list_filtered_AMI)
...
# Remove the Openness scale
refined_scales_list_filtered_ASI$Openness <- NULL
refined_scales_list_filtered_AMI$Openness <- NULL
...
vif_factor <- function(data, factor_items) {
  ...
  # Compute VIF for each factor using the combined_refined_scales_list object
vif_results <- list()

for(factor_name in names(combined_refined_scales_list)) {
  factor_items <- combined_refined_scales_list[[factor_name]]
...
summary_AMI_personality <- summary(fit_AMI_personality, fit.measures=TRUE, rsquare = TRUE)
summary_AMI_SocialPsychology <- summary(fit_AMI_SocialPsychology, fit.measures=TRUE, rsquare = TRUE)
summary_AMI_Combined <- summary(fit_AMI_Combined, fit.measures=TRUE, rsquare = TRUE)
summary_ASI_personality <- summary(fit_ASI_personality, fit.measures=TRUE, rsquare = TRUE)
summary_ASI_SocialPsychology <- summary(fit_ASI_SocialPsychology, fit.measures=TRUE, rsquare = TRUE)
summary_ASI_Combined <- summary(fit_ASI_Combined, fit.measures=TRUE, rsquare = TRUE)
...
anova(fit_AMI_personality, fit_AMI_SocialPsychology)
anova(fit_AMI_personality, fit_AMI_Combined)
anova(fit_AMI_Combined, fit_AMI_SocialPsychology)

anova(fit_ASI_personality, fit_ASI_SocialPsychology)
anova(fit_ASI_personality, fit_ASI_Combined)
anova(fit_ASI_Combined, fit_ASI_SocialPsychology)

anova(fit_ASI_Combined, fit_AMI_Combined)


## RERUN CORRELATIONS AND DESCRIPTIVES FOR NEW FACTOR STRUCTURE

# Create and save composite scales based on combined_refined_scales_list
for (scale_name in names(combined_refined_scales_list)) {
  items <- combined_refined_scales_list[[scale_name]]
  
  # Calculate the composite scale and save it with a different name
  composite_scale <- rowMeans(psp_data[, items], na.rm = TRUE)
  new_scale_name <- paste0(scale_name, "_composite")
  
  # Save the composite scale in psp_data
  psp_data[, new_scale_name] <- composite_scale
}

# List of variables for correlation and descriptives
variables <- c("AMI_composite", "ASI_composite", "SDO_composite", 
               "RWA_composite", "Agreeableness_composite", 
               "Gender")

# Calculate correlation matrix
correlation_matrix <- cor(psp_data[variables], use = "complete.obs")

# Initialize an empty matrix for p-values
p_values_matrix <- matrix(NA, nrow = length(variables), ncol = length(variables))
rownames(p_values_matrix) <- colnames(p_values_matrix) <- variables

# Calculate correlation coefficients and p-values
for (i in 1:(length(variables) - 1)) {
  for (j in (i + 1):length(variables)) {
    cor_test <- cor.test(psp_data[[variables[i]]], psp_data[[variables[j]]], 
                         method = "pearson", use = "complete.obs")
    p_values_matrix[i, j] <- cor_test$p.value
    p_values_matrix[j, i] <- cor_test$p.value
  }
}
...
correlation_df_final <- as.data.frame(correlation_matrix)
row.names(correlation_df_final) <- colnames(correlation_df_final) <- variables

# Calculate means and standard deviations
means <- colMeans(psp_data[variables], na.rm = TRUE)
std_devs <- apply(psp_data[variables], 2, sd, na.rm = TRUE)

# Create dataframe for means and standard deviations
descriptives_df_final <- data.frame(Mean = means, SD = std_devs)
row.names(descriptives_df_final) <- variables

# Print correlation dataframe with p-values
print("Correlation Matrix:")
print(correlation_df_final)

# Print p-values dataframe
print("P-Values Matrix:")
print(p_values_df)

# Print means and standard deviations dataframe
print("Means and Standard Deviations:")
print(descriptives_df_final)

#### EXPORT RESULTS

# Extract standardized solutions for each model
std_AMI_CFA <- standardizedSolution(measurement_fit_AMI_refined)
std_ASI_CFA <- standardizedSolution(measurement_fit_ASI_refined)
std_AMI_personality <- standardizedSolution(fit_AMI_personality)
std_AMI_SocialPsychology <- standardizedSolution(fit_AMI_SocialPsychology)
std_AMI_Combined <- standardizedSolution(fit_AMI_Combined)
std_ASI_personality <- standardizedSolution(fit_ASI_personality)
std_ASI_SocialPsychology <- standardizedSolution(fit_ASI_SocialPsychology)
std_ASI_Combined <- standardizedSolution(fit_ASI_Combined)

df_AMI_CFA <- as.data.frame(std_AMI_personality)
df_ASI_CFA <- as.data.frame(std_ASI_SocialPsychology)
df_AMI_personality <- as.data.frame(std_AMI_personality)
df_AMI_SocialPsychology <- as.data.frame(std_AMI_SocialPsychology)
df_AMI_Combined <- as.data.frame(std_AMI_Combined)
df_ASI_personality <- as.data.frame(std_ASI_personality)
df_ASI_SocialPsychology <- as.data.frame(std_ASI_SocialPsychology)
df_ASI_Combined <- as.data.frame(std_ASI_Combined)
...
dataframes <- list(
  "Descriptives by Category" = descriptives_by_category,
  "Descriptives Scales" = descriptives_scales,
  "Sample Statistics" = sample_stats,
  "CFA AMI" = df_AMI_CFA,
  "CFA ASI" = df_ASI_CFA,
  "AMI Personality" = df_AMI_personality,
  "AMI SocialPsychology" = df_AMI_SocialPsychology,
  "AMI Combined" = df_AMI_Combined,
  "ASI Personality" = df_ASI_personality,
  "ASI SocialPsychology" = df_ASI_SocialPsychology,
  "ASI Combined" = df_ASI_Combined,
  "Initial Correlations" = correlation_dataframe_result,
  "Final Correlations" = correlation_df_final,
  "Correlations p-values" = p_values_df,
  "Final Scale Descriptives" = descriptives_df_final
)

# Define the output Excel file name
output_file <- "Results.xlsx"

# Export the dataframes to the Excel file
write_xlsx(dataframes, path = output_file)
...
# Assuming fit_AMI_personality is your fitted SEM model
paths <- semPaths(
  fit_AMI_personality,
  layout = "tree",
  whatLabels = "std",
...
semPaths(fit_ASI_personality, whatLabels = "est")
