setwd("C:/Users/rafre/Dropbox/Fiverr/Trabalhos/2023/getsellgo")

library(readxl)
df <- read_xlsx('WIPO_4_Question_Survey_Results_Data_051213.xlsx', sheet = 'FOUR Q DATA')
...
df_freq_ANS_C <- calculate_freq_percent(df$ANS_C, "ANS_C")
...
all_freq_data <- bind_rows(df_freq_rating_changed, df_freq_rating_change, df_freq_quizscore, df_freq_ans_s, df_freq_ans_d, df_freq_ANS_C, count_data, df_freq_rating_before, df_freq_rating_after)
...
ggplot(long_df, aes(x = QUIZ_SCORE, y = Mean, group = Timepoint, color = Timepoint)) +
  geom_line() +
  geom_point() +
  geom_errorbar(data = means_ci_df, aes(x = QUIZ_SCORE, ymin = Lower_CI_Before, ymax = Upper_CI_Before), color = "gray", width = 0.1, inherit.aes = FALSE) +
  geom_errorbar(data = means_ci_df, aes(x = QUIZ_SCORE, ymin = Lower_CI_After, ymax = Upper_CI_After), color = "gray", width = 0.1, inherit.aes = FALSE) +
...
dependent_variable <- "RATING_CHANGE_NUM"  # Replace with your dependent variable name
independent_variables <- c("ANS_S", "ANS_D", "ANS_C", "RATING_BEFORE")  # Replace with your independent variable names
...
# Ensure that ANS_S, ANS_D, ANS_C, and QUIZ_SCORE are factors
df$ANS_S <- as.factor(df$ANS_S)
df$ANS_D <- as.factor(df$ANS_D)
df$ANS_C <- as.factor(df$ANS_C)
df$QUIZ_SCORE <- as.factor(df$QUIZ_SCORE)
...
factors <- list(ANS_S = levels(df$ANS_S), ANS_D = levels(df$ANS_D), ANS_C = levels(df$ANS_C), QUIZ_SCORE = levels(df$QUIZ_SCORE))
