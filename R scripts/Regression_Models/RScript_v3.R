...
# Initialize an empty data frame to store the mean values and standard errors
mean_values_df <- data.frame()

# Loop through each dependent variable
for (var in dvs) {
  
  # Calculate mean scores and standard errors for each group and timepoint
  temp_df <- df %>%
    group_by(!!sym(between_subject_factor), !!sym(within_subject_factor)) %>%
    summarise(
      Mean_Value = mean(!!sym(var), na.rm = TRUE),
      SE_Value = sd(!!sym(var), na.rm = TRUE) / sqrt(n()),
      .groups = "drop"  # To suppress the warning
    ) %>%
    pivot_wider(names_from = !!sym(within_subject_factor), 
                values_from = c(Mean_Value, SE_Value)) %>% # reshape using pivot_wider
    add_column(Variable = var, .before = 1) # add the dependent variable name
  
  # Add the mean values and standard errors to the final data frame
...
write.xlsx(mean_values_df, "mean_values_output.xlsx", rowNames = FALSE)
...
for (var in dvs) {
  ...
  for (i in 1:(length(group_names) - 1)) {
    for (j in (i + 1):length(group_names)) {
      ...
      # New loop for each unique pair of time points
      time_points <- unique(df$Timepoint)
      for (m in 1:(length(time_points) - 1)) {
        for (n in (m + 1):length(time_points)) {
          
          # Filter data for the current pair of groups and time points
          df_pair <- df %>% 
            filter(Group.Name %in% c(group_names[i], group_names[j])) %>%
            filter(Timepoint %in% c(time_points[m], time_points[n]))
          ...
          model_summary_temp <- data.frame(
            ...
            Timepoint_Pair = paste(time_points[m], "-", time_points[n]),  # New column
...
for (cutoff_age in c(48, 50, 52, 54, 56, 58, 60)) {
  ...
  # Initialize an empty dataframe to store model summary statistics
  model_summary_df_age <- data.frame(
    Dependent_Variable=character(), 
    Effect=character(),
    numDF=numeric(),
    denDF=numeric(),
    ...
    for (i in 1:(length(group_names) - 1)) {
      for (j in (i + 1):length(group_names)) {
        ...
        for (var in dvs) {
          ...
          # Loop through each unique pair of time points for post-hoc tests
          for (m in 1:(length(time_points) - 1)) {
            for (n in (m + 1):length(time_points)) {
              
              # Filter data for the current pair of time points
              df_pair_time <- df_pair %>% filter(Timepoint %in% c(time_points[m], time_points[n]))
              
              # Fit the linear mixed-effects model using nlme
              model_formula <- as.formula(paste(var, "~ Group.Name * Timepoint"))
              model <- lme(model_formula, random = ~1 | Subject, data = df_pair_time, method = "REML")
              ...
              model_summary_temp <- data.frame(
                ...
                Age_Group = age_group,
                Group_Pair = paste(group_names[i], "-", group_names[j]),
                Timepoint_Pair = paste(time_points[m], "-", time_points[n]),
                Age_Cutoff = cutoff_age,
                stringsAsFactors = FALSE
              )
...
# Initialize an empty dataframe to store the mean scores and standard errors
...
for (cutoff in cutoff_points) {
  ...
  for (var in dvs) {
    ...
    # Calculate the mean scores and standard errors for each DV, Timepoint, Group.Name, and Age_Group
    mean_scores_temp <- df %>%
      group_by(Timepoint, Group.Name, Age_Group) %>%
      summarise(
        Mean_Score = mean(!!sym(var), na.rm = TRUE),
        SE_Score = sd(!!sym(var), na.rm = TRUE) / sqrt(n()),
        Age_Cutoff = cutoff,
        Dependent_Variable = var,
        .groups = "drop"  # To suppress the warning about grouped output
      )
    
    # Append the temporary mean scores and standard errors to the main dataframe
    mean_scores_age_df <- rbind(mean_scores_age_df, mean_scores_temp)
  }
}

# Display the updated dataframe with mean scores and standard errors
print(mean_scores_age_df)

df_age_groups_SE <- mean_scores_age_df %>%
  pivot_wider(
    id_cols = c(Group.Name, Age_Group, Age_Cutoff, Dependent_Variable),
    names_from = Timepoint,
    values_from = c(Mean_Score, SE_Score),
    names_sep = "_"
  )

write.xlsx(df_age_groups_SE, "mean_values_output_Se_Ages.xlsx", rowNames = FALSE)
...
mean_values_long <- mean_values_df %>%
  gather(key = "Timepoint", value = "Mean_Value_", T0, T42, T84)
...
# Export model Summaries for all posthocs

# Create a new workbook
wb <- createWorkbook()

# Add a worksheet for siginteraction_filtered_df_age
addWorksheet(wb, "ModelSummary")
writeData(wb, "ModelSummary", model_summary_df)

# Add a worksheet for siginteraction_filtered_df
addWorksheet(wb, "ModelSummary_Age")
writeData(wb, "ModelSummary_Age", model_summary_df_age)

# Save the workbook
saveWorkbook(wb, "postHoc Summaries_v2.xlsx", overwrite = TRUE)
