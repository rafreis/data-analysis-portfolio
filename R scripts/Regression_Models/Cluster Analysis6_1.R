...
perform_hierarchical_clustering <- function(df, vars) {
  ...
  for (k in 2:10) {
    ...
    # Gap Statistic
    set.seed(123)
    gap_stat <- clusGap(data_selected, FUNcluster = kmeans, K.max = 10, B = 50)
    
    # Silhouette analysis
    sil_width <- mean(silhouette(cluster_assignments, dissimilarity_matrix)[, 'sil_width'])
    
    # Store results
    elbow_df_list[[k]] <- data.frame(Number_of_Clusters = k, Total_withinss = wss)
    sil_df_list[[k]] <- data.frame(Number_of_Clusters = k, Average_Silhouette_Width = sil_width)
    gap_df_list[[k]] <- data.frame(Number_of_Clusters = 1:10, Gap_Stat = gap_stat$Tab[, "gap"])
    perform_hierarchical_clustering <- function(df, vars) {
      # Select variables
      data_selected <- df %>% select(all_of(vars))
      ...
      elbow_df <- do.call(rbind, elbow_df_list)
      sil_df <- do.call(rbind, sil_df_list)
...
factors <- clusterassignmentcolumn
...
segmented_freq_df <- create_segmented_frequency_tables(df_withclusters, categories_clustervars, clusterassignmentcolumn)
...
dot_whiskers_plot <- create_dot_and_whiskers_plot(df_withclusters, clusterassignmentcolumn, vars_to_plot1)
dot_whiskers_plot2 <- create_dot_and_whiskers_plot(df_withclusters, clusterassignmentcolumn, vars_to_plot2)
# Display the plot
...
results_anova_clusters <- perform_anova_and_posthoc(df_withclusters, vars_to_plot, clusterassignmentcolumn)
