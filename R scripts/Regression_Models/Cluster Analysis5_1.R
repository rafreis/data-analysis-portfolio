...
# 5-clusters is the optimal solution - Cluster PROFILING
...
clusterassignmentcolumn <- "Cluster_5"
...
segmented_freq_df <- create_segmented_frequency_tables(df_withclusters, categories_clustervars, "Cluster_5")
...
create_dot_and_whiskers_plot <- function(df, cluster_var, vars_to_plot) {
  ...
  ggplot(mean_ci_df, aes(x = Variable, y = Mean, group = Cluster, color = as.factor(Cluster))) +
    ...
    labs(title = "Dot and Whiskers Plot of Means by Cluster",
         x = "Variable", y = "Mean", color = "Cluster") +
    theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1))
}
...
vars_to_plot <- c("Pro_technology", "Pro_environment", "Public_transport_attitude", 
                   "Performance_expectancy", "Effort_expectancy","Social_Influence", 
                  "Facilitating_Conditions", "Hedonic_Motivation", "Price_value", 
                  "Perceived_risks", "Behavioural_Intention")


dot_whiskers_plot <- create_dot_and_whiskers_plot(df_withclusters, "Cluster_5", vars_to_plot1)
dot_whiskers_plot2 <- create_dot_and_whiskers_plot(df_withclusters, "Cluster_5", vars_to_plot2)
# Display the plot
...
results_anova_clusters <- perform_anova_and_posthoc(df_withclusters, vars_to_plot, "Cluster_5")
...
data_list <- list(
  ...
  "Elbow Method" = Elbow_df,
  "Gap Statistic" = GapStat_df,
  "Silhouette Statistics" = Silhouette_df,
...
check_multicollinearity <- function(df, variables) {
  vif_results <- data.frame(Variable = character(), VIF = numeric(), Tolerance = numeric(), stringsAsFactors = FALSE)
  
  for (var in variables) {
    # Constructing formula for each variable as DV
    dv <- var
    ivs <- setdiff(variables, var) # All other variables as IVs
    formula <- as.formula(paste(dv, "~", paste(ivs, collapse = "+")))
    
    # Try-catch to handle potential errors in model fitting or VIF calculation
    tryCatch({
      lm_model <- lm(formula, data = df)
      # Calculate VIF for the model, ensuring we target the correct variable
      # When calculating VIF, it calculates for all IVs in the model, so we directly use the vif() function output
      vif_values <- vif(lm_model)
      
      # Since var is used as DV, its VIF is not directly calculated, so we handle it indirectly through the model on other IVs
      # Append the results for each IV variable
      for (iv_var in names(vif_values)) {
        vif_results <- rbind(vif_results, data.frame(Variable = iv_var, VIF = vif_values[iv_var], Tolerance = 1/vif_values[iv_var]))
      }
    }, error = function(e) {
      message("Error processing variable: ", var, "; Error: ", e$message)
    })
  }
  
  # Since each IV gets listed with each DV iteration, we might have duplicate entries; we summarize it
  aggregate(cbind(VIF, Tolerance) ~ Variable, data = vif_results, mean)
}

variables <- c("Pro-technology" ,                                                           
"Pro-environment"                ,                                           
"Public transport attitude"       ,                                          
"Performance expectancy"           ,                                         
"Effort expectancy"                 ,                                        
...
# Base R approach to rename variables, replacing spaces and hyphens with underscores
names(df_recoded) <- gsub("[- ]", "_", names(df_recoded))

# Updating the variables list to match the new naming convention
variables_updated <- gsub("[- ]", "_", variables)

# Removing rows with NAs only in the variables of interest
df_recoded_clean <- df_recoded[complete.cases(df_recoded[variables_updated]), ]

# You can then proceed to use the updated variables with the check_multicollinearity function
results <- check_multicollinearity(df_recoded_clean, variables_updated)
