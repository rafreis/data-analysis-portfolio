...
reverse_scores_multiple_columns <- function(df, column_names) {
  ...
  # Also preserve any existing column labels
  for (column_name in column_names) {
    ...
    # Check if the original column has labels and preserve them if it does
    if ("label" %in% attr(df[[column_name]], "class")) {
      # Copy labels from the original column to the new reversed column
      attr(df[[new_column_name]], "label") <- attr(df[[column_name]], "label")
    }
  }
...
# Combining the reliability tables into one using bind_rows
df_reliability_combined <- bind_rows(df_reliability_attitudes, df_reliability_drugsupport, df_reliability_policysupport)
...
vars_clustering <- vars_attitudestodrugs
...
# Join Cluster assignments back to df

df_with_clusters_selected <- df_withclusters %>%
  select(RecallCode, Cluster_3, Cluster_4, Cluster_5, Cluster_6, Cluster_7, Cluster_8)

# Now perform the join with the selected columns
df <- df %>%
  left_join(df_with_clusters_selected, by = "RecallCode")
...
anova_results_3clusters <- run_anova(df, "Cluster_3", vars_clustering)
anova_results_4clusters <- run_anova(df, "Cluster_4", vars_clustering)
anova_results_5clusters <- run_anova(df, "Cluster_6", vars_clustering)
anova_results_6clusters <- run_anova(df, "Cluster_6", vars_clustering)
anova_results_7clusters <- run_anova(df, "Cluster_7", vars_clustering)
anova_results_8clusters <- run_anova(df, "Cluster_8", vars_clustering)
...
df_nohaven <- df %>%
  mutate(across(all_of(c(vars_attitudestodrugs,vars_drugsupport,vars_supportpolicies)), ~ as.numeric(as.vector(.))))

anova_results_nonclustvars <- run_anova(df_nohaven, "Cluster_6", c(vars_attitudestodrugs,vars_drugsupport,vars_supportpolicies))
...
df_clustersizes <- calculate_cluster_sizes(df, clusterassignmentcolumn)
...
factors <- c("Cluster_6")
df_descriptive_clustervars_bycluster <- calculate_means_and_sds_by_factors(df, numbers_clustervars, factors)

numbers_clustervars2 <- c(vars_drugsupport, vars_supportpolicies, vars_attitudestodrugs)
...
create_segmented_frequency_tables <- function(data, vars, segmenting_categories) {
  ...
  for (segment in segmenting_categories) {
    ...
    for (var in vars) {
      ...
      # Calculate percentages relative to each segment and Variable
      freq_table$Percentage <- ave(freq_table$Count, freq_table$Segment, FUN = function(x) (x / sum(x)) * 100)
  ...
  # Combine all frequency tables into a single dataframe
  combined_freq_tables <- do.call(rbind, all_freq_tables)
  
  # Reset row names to avoid duplicate row names
  rownames(combined_freq_tables) <- NULL
  
  return(combined_freq_tables)
}



df_segmented_freq <- create_segmented_frequency_tables(df, categories_clustervars, "Cluster_6")

df_nohaven <- df %>%
  mutate(across(all_of(vars_comparison), ~ as.numeric(as.vector(.))))

df_freq_media <- create_segmented_frequency_tables(df_nohaven, vars_comparison, "Cluster_6")

df_freq_demo_all <- create_frequency_tables(df_nohaven, vars_comparison)
...
df_descriptive_detclustervars_bycluster <- calculate_means_and_sds_by_factors(df, numbers_clustervars2, factors)
...
dot_whiskers_plot <- create_dot_and_whiskers_plot(df, "Cluster_6", vars_to_plot1)
dot_whiskers_plot2 <- create_dot_and_whiskers_plot(df, "Cluster_6", vars_to_plot2)
dot_whiskers_plot3 <- create_dot_and_whiskers_plot(df, "Cluster_6", vars_to_plot3)
dot_whiskers_plot4 <- create_dot_and_whiskers_plot(df, "Cluster_6", vars_to_plot4)
...
df_nohaven <- df_with_clusters_list$Cluster_8
...
df_chisq6 <- chi_square_analysis_multiple(df, row_variables, column_variable)
column_variable3 <- "Cluster_6"
df_chisq3 <- chi_square_analysis_multiple(df, row_variables, column_variable3)


# Labels and names mapping

library(dplyr)
library(haven)

# Assuming 'df' is your dataframe and it might have variables imported with 'haven' that contain labels
# Initialize an empty dataframe to store column names and their labels
column_labels <- data.frame(ColumnName = character(), Label = character(), stringsAsFactors = FALSE)

# Iterate over the columns of 'df'
for (column_name in names(df)) {
  # Attempt to retrieve the label of the current column, if it exists
  column_label <- attr(df[[column_name]], "label")
  
  # If the column does not have a label, you might choose to represent this as NA or a blank string
  if (is.null(column_label)) {
    column_label <- NA  # or "", depending on your preference for representing unlabeled columns
  }
  
  # Append the current column's name and label to the 'column_labels' dataframe
  column_labels <- rbind(column_labels, data.frame(ColumnName = column_name, Label = column_label, stringsAsFactors = FALSE))
}

# View the resulting mapping of column names to labels
print(column_labels)
...
# Example usage
data_list <- list(
  "Dataset with Clusters" = df_withclusters,
  "Frequency - Demog" = df_freq_demo2,
  "Frequency - Media Use" = df_freq_demo_all,
  "Frequency - Demog per Cluster" = df_segmented_freq,
  "Frequency - Media per Cluster" = df_freq_media,
  "Reliability - Attitudes" = df_reliability_combined,
  ...
  "ANOVA Results - Cluster Only" = anova_results_6clusters,
  "ANOVA Results - NonCluster" = anova_results_nonclustvars,
  "Label mapping" = column_labels
)

# Save to Excel with APA formatting
save_apa_formatted_excel(data_list, "APA_Formatted_Tables_Final2.xlsx")
...
# Formula for the LDA
formula <- as.formula(paste(cluster_col, "~", paste(vars_clustering, collapse = "+")))
      
# Perform Linear Discriminant Analysis
lda_model <- lda(formula, data = data)
      
# Discriminatory power of each variable (standardized coefficients)
discriminatory_power <- as.data.frame(lda_model$scaling)



# Access the results
discriminatory_power_df <- results$DiscriminatoryPower
group_means_df <- results$GroupMeans
coefficients_df <- results$Coefficients


# Example usage
data_list <- list(
  "Discriminant Function" = discriminatory_power
)

# Save to Excel with APA formatting
save_apa_formatted_excel(data_list, "APA_Formatted_Tables_DiscAnalysis.xlsx")
