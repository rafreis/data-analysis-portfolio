...
# Define dichotomies
df$usage_cutoff_A <- factor(ifelse(df$QID49 %in% 1:4, "High", ifelse(df$QID49 == 5, "Low", NA)),
                            levels = c("Low", "High"))
df$usage_cutoff_B <- factor(ifelse(df$QID49 %in% 1:3, "High", ifelse(df$QID49 %in% 4:5, "Low", NA)),
                            levels = c("Low", "High"))

# Update Levene's and Kruskal–Wallis to support alternative cutoff
run_tests_for_cutoff <- function(data, usage_var_label, usage_var) {
  # Levene's Test
  levene_res <- perform_levenes_test(data, response_vars, c(usage_var))
  levene_res$Cutoff <- usage_var_label
  
  # Kruskal–Wallis Test
  kruskal_res <- perform_kruskal_test(data, response_vars, c(usage_var))
  kruskal_res$Cutoff <- usage_var_label
  
  return(list(levene = levene_res, kruskal = kruskal_res))
}

# Run for both cutoff types
results_A <- run_tests_for_cutoff(df, "Cutoff_A (1–4 vs 5)", "usage_cutoff_A")
results_B <- run_tests_for_cutoff(df, "Cutoff_B (1–3 vs 4–5)", "usage_cutoff_B")

# Combine results
levene_combined <- bind_rows(results_A$levene, results_B$levene)
kruskal_combined <- bind_rows(results_A$kruskal, results_B$kruskal)

# Optional: save them into Excel
save_apa_formatted_excel(
  list(
    "Levene Tests Cutoffs" = levene_combined,
    "Kruskal-Wallis Cutoffs" = kruskal_combined
  ),
  "ARVR_GroupComparison_Cutoffs_APA.xlsx"
)
...
# Label dictionary for titles and axis labels
label_dict <- c(
  Q51 = "Collaboration (Team)",
  Q54 = "Decision-Making (Design)",
  SelfEfficacy_ARVR = "Self-Efficacy with AR/VR",
  WorkflowEfficiency = "Workflow Efficiency",
  DecisionMakingConfidence = "Confidence in Complex Challenges",
  usage_dicho_v1 = "AR/VR Usage (1–4 vs 5)",
  usage_dicho_v2 = "AR/VR Usage (1–3 vs 4–5)"
)

# Create dichotomized usage variables
df$usage_dicho_v1 <- factor(ifelse(df$QID49 %in% c(1, 2, 3, 4), "High", "Low"), levels = c("Low", "High"))
df$usage_dicho_v2 <- factor(ifelse(df$QID49 %in% c(1, 2, 3), "High", "Low"), levels = c("Low", "High"))

response_vars <- c("Q51", "Q54", "SelfEfficacy_ARVR", "WorkflowEfficiency", "DecisionMakingConfidence")
dichos <- c("usage_dicho_v1", "usage_dicho_v2")



# Function to generate plots for each DV and dichotomized IV
for (dv in response_vars) {
  for (dicho in dichos) {
    
    # BOXPLOT
    g_box <- ggplot(df, aes_string(x = dicho, y = dv)) +
      geom_boxplot(fill = "#cccccc", outlier.shape = NA, width = 0.5) +
      geom_jitter(width = 0.15, alpha = 0.6, size = 1.5) +
      theme_minimal(base_size = 14) +
      labs(
        title = paste("Group Differences in", label_dict[[dv]]),
        x = label_dict[[dicho]],
        y = label_dict[[dv]]
      ) +
      theme(plot.title = element_text(hjust = 0.5))
    
    ggsave(paste0("plots_boxplots/box_", dv, "_", dicho, ".png"), g_box, width = 8, height = 6, dpi = 300)
    
    # DENSITY PLOT
    g_dens <- ggplot(df, aes_string(x = dv, fill = dicho)) +
      geom_density(alpha = 0.4) +
      theme_minimal(base_size = 14) +
      labs(
        title = paste("Density Plot of", label_dict[[dv]], "by", label_dict[[dicho]]),
        x = label_dict[[dv]],
        y = "Density",
        fill = label_dict[[dicho]]
      ) +
      theme(plot.title = element_text(hjust = 0.5))
    
    ggsave(paste0("plots_density/dens_", dv, "_", dicho, ".png"), g_dens, width = 8, height = 6, dpi = 300)
    
    # DOT AND WHISKER PLOT
    df_summary <- df %>%
      group_by(across(all_of(dicho))) %>%
      summarise(
        Mean = mean(get(dv), na.rm = TRUE),
        SEM = sd(get(dv), na.rm = TRUE) / sqrt(sum(!is.na(get(dv)))),
        .groups = "drop"
    ...
    colnames(df_summary)[1] <- "Group"
    
    g_dot <- ggplot(df_summary, aes(x = Group, y = Mean)) +
      geom_point(size = 3) +
      geom_errorbar(aes(ymin = Mean - SEM, ymax = Mean + SEM), width = 0.15, linewidth = 0.8) +
      theme_minimal(base_size = 14) +
      labs(
        title = paste("Mean ± SEM for", label_dict[[dv]]),
        x = label_dict[[dicho]],
        y = label_dict[[dv]]
      ) +
      theme(plot.title = element_text(hjust = 0.5))
    
    ggsave(paste0("plots_dotwhisker/dot_", dv, "_", dicho, ".png"), g_dot, width = 8, height = 6, dpi = 300)
  }
}
