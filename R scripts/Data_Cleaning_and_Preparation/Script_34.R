...
plot_histogram <- function(data, variable_name, simple_name) {
  # Filter data for the specific variable
  filtered_data <- data %>% filter(Variable == variable_name)
  
  # Generate histogram
  plot <- ggplot(filtered_data, aes(x = Percentage_Error, fill = Variable)) +
    geom_histogram(bins = 50, alpha = 0.6, position = "identity") +  # Increased number of bins
    scale_fill_manual(values = c("skyblue")) +
    labs(title = paste("Distribution of Percentage Errors -", simple_name),
         x = "Percentage Error (%)",
         y = "Frequency",
         fill = "Error Type") +
    theme_minimal() +
...
for (var in variables) {
  plot_histogram(long_data, var$name, var$simple)
}
