...
result_df_dummy$Row_Variable <- gsub("[._]", " ", result_df_dummy$Row_Variable)

# Ensure 'Column_Variable' is treated as a factor
result_df_dummy$Column_Variable <- factor(result_df_dummy$Column_Variable, levels = unique(result_df_dummy$Column_Variable))

# Assuming result_df_rowperc is your dataframe
result_df_dummy$Row_Level <- ifelse(result_df_dummy$Row_Level == '1', 'Yes',
                                    ifelse(result_df_dummy$Row_Level == '0', 'No', result_df_dummy$Row_Level))

result_df_dummy$Column_Variable <- factor(result_df_dummy$Column_Variable, levels = c("1 year", "2 years", "3 years", "4 years"))

# Unique Row_Variables
unique_row_vars <- unique(result_df_dummy$Row_Variable)
