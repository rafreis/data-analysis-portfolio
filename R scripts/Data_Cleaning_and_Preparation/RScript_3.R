...
us_map <- st_read("States_shapefile/states_shapefile.shp")
us_map_merged <- left_join(us_map, df_by_state, by = c("State_Code" = "State"))

ggplot(data = us_map_merged) +
  geom_sf(aes(fill = Sample_Size)) +
  scale_fill_gradient(low = "white", high = "blue") +  # change colors based on your preference
  theme_minimal() +
  labs(title = "Your Title", 
       subtitle = "Your Subtitle", 
       caption = "Your Caption", 
       fill = "Your Legend Title")
