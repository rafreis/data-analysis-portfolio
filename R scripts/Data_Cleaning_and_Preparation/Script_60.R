...
setwd("C:/Users/rafre/Dropbox/Fiverr/Trabalhos/2024/trackrecord/Third Wave")


#gs4_auth()  # This will open a browser window for you to authenticate

# Specify the Google Sheet URL
sheet_url <- "https://docs.google.com/spreadsheets/d/1skCqSH7RFqMt4q7UmxIZP3y6D0ajEjvyXobwGJ3NM1c/edit?gid=501565844#gid=501565844"

# Get all sheet names
sheet_names <- sheet_names(sheet_url)

# Select only the first 18 sheet names
sheet_names <- sheet_names(sheet_url)
...
# Read data from the first 18 sheets and combine into a single dataframe
...
library(dplyr)
# Using dplyr to get distinct values
distinct_leaders <- all_data %>%
  distinct(Leader) %>%
  arrange(Leader)  # Optional, to sort the leaders alphabetically

# To view the distinct leaders
...
create_mean_ci_plots <- function(data, variables, factor_column) {
  ...
  # Creating a directory to save the plots if it does not exist
  plot_directory <- "mean_ci_plots"
  ...
  for (variable in variables) {
    ...
    # Define the filename
    file_name <- paste0(plot_directory, "/", gsub("[[:punct:] ]+", "_", variable), ".png")
...
# Updated leader groups based on provided categories
emea_sales_leaders <- c(
  "LA Brian Slattery",
  "Josef Sommeregger"
)
...
create_mean_ci_plots(df_recoded_nooutliers, scales_scores, leader_col, "EMEA and Sales", emea_sales_leaders)
create_mean_ci_plots(df_recoded_nooutliers, scales_scores, leader_col, "NA and Sales", na_sales_leaders)
create_mean_ci_plots(df_recoded_nooutliers, scales_scores, leader_col, "Marketing", marketing_leaders)
...
# Assuming 'leader_col' is already set to a column that classifies each leader into one of the new groups
create_column_plots(df_recoded_nooutliers, scales_scores, leader_col)
...
# Calculate descriptive statistics for each group, assuming you have a 'Group' column correctly set
df_descriptive_stats <- calculate_descriptive_stats(df_recoded_nooutliers, scales_scores)

# Additionally, you could calculate means and standard deviations by factors if 'Group' is correctly defined
...
# Assuming the heatmap function is properly defined and you have a 'Group' column
create_heatmap(df_recoded_nooutliers, "EMEA / Sales", emea_sales_leaders)
create_heatmap(df_recoded_nooutliers, "NA / Sales", na_sales_leaders)
create_heatmap(df_recoded_nooutliers, "Marketing", marketing_leaders)
...
df_scaled <- df_relevant %>%
  mutate(across(-Leader, ~((. - 1) / 3 * 100)))
...
# Define the leaders for each area

hife_leaders <- c("Jennifer Bringman", "Stacy Whitman", "Tanuja Gairola", "Gianfranco Aragona")
ops_leaders <- c("Benny Johnson", "Emily Brawley", "Jessica Straughn", "Colin Clunie", 
                 "Kai Applequist", "Molly Petrowiak", "Deepika Murthy", "Neeraj Pandey", 
                 "Brian Stephens", "Pratima Kaul", "Luke Austin", "Lauren Thueson", 
                 "Seth Vega", "Matt Fedrizzi")

df_final_2 <- df_final_2 %>%
  mutate(Leader = case_when(
    Leader == "LA Emily Brawley" ~ "Emily Brawley",
    Leader == "LA Brian Stephens" ~ "Brian Stephens",
    Leader == "LA Benny Johnson" ~ "Benny Johnson",
    TRUE ~ Leader  # Keeps all other leader names unchanged
  ))

# Assume df_final_2 already exists and has a column 'Leader' with leader names
df_final_2 <- df_final_2 %>%
  mutate(Area = case_when(
    Leader %in% hife_leaders ~ "HIFE",
    Leader %in% ops_leaders ~ "Ops",
    TRUE ~ "Other"  # Assigns 'Other' if leader not found in the provided lists
  ))
...
# Function to create mean and 95% CI plot for a specific group
create_mean_ci_plots <- function(data, variables, factor_column, area_column) {
  # Filter the data for the specified area
  # This assumes the data is already filtered or you want to plot for all areas
  group_data <- data
  
  # Create a long format of the data for ggplot
  long_data <- group_data %>%
    pivot_longer(cols = all_of(variables), names_to = "Variable", values_to = "Value") %>%
    group_by(!!sym(area_column), !!sym(factor_column), Variable) %>%
    summarise(
      Mean = mean(Value, na.rm = TRUE),
      N = n(),
      SD = sd(Value, na.rm = TRUE),
  ...
  # Loop through each area to create separate plots for each variable
  area_levels <- unique(group_data[[area_column]])
  for (area in area_levels) {
    for (variable in variables) {
      plot_data <- long_data %>% filter(Variable == variable & !!sym(area_column) == area)
      p <- ggplot(plot_data, aes(x = !!sym(factor_column), y = Mean, group = !!sym(area_column))) +
        geom_point(aes(color = !!sym(area_column)), size = 3) +
        geom_errorbar(aes(ymin = Lower, ymax = Upper, color = !!sym(area_column)), width = 0.2) +
        theme(
          axis.text.x = element_text(angle = 90, vjust = 0.5),
          legend.position = "right"
        ) +
        labs(
          title = paste("Mean and 95% CI for", variable, "in", area), 
          x = factor_column,
          y = "Value"
        ) +
        scale_y_continuous(limits = c(0, 100), breaks = seq(0, 100, by = 20))
      ...
      # Save the plot
      ggsave(filename = paste0("mean_ci_plot_", variable, "_", area, ".png"), plot = p, width = 10, height = 6)
    }
  }
}
...
create_mean_ci_plots(df_final_2, scales_scores, leader_col, area_col)
