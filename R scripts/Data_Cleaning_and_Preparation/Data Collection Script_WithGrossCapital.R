...
# Define the indicators to retrieve, including GFCF
indicators <- c(FDI = "BX.KLT.DINV.WD.GD.ZS",     # Foreign direct investment, net inflows (% of GDP)
                ...
                GFCF = "NE.GDI.FTOT.ZS")           # Gross fixed capital formation (% of GDP)
...
df <- df %>%
  group_by(country) %>%
  mutate(
    FDI_lag1 = lag(FDI, 1),   # Creates a lag of 1 year for FDI
    FDI_lag2 = lag(FDI, 2)    # Creates a lag of 2 years for FDI
  )
...
write.csv(df, "cleaned_WDI_data_for_SPSS_withGFCF.csv", row.names = FALSE)
