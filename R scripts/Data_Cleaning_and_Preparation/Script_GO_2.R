...
df_recoded <- all_data %>%
  mutate(
    `This function has a clear and inspiring vision` = recode_responses(`This function has a clear and inspiring vision`),
    `This function has credibility and a well-defined role across the organization` = recode_responses(`This function has credibility and a well-defined role across the organization`),
    `This function has the leadership team to deliver effectively to achieve its potential` = recode_responses(`This function has the leadership team to deliver effectively to achieve its potential`),
    `This function has a direct impact and contribution to Clearwater achieving its goals around growth and innovation.` = recode_responses(`This function has a direct impact and contribution to Clearwater achieving its goals around growth and innovation.`),
    `This function has the team and resources to achieve its goals` = recode_responses(`This function has the team and resources to achieve its goals`)
  )
...
df_final <- df_final %>%
  mutate(
    `This function has a clear and inspiring vision` = rescale_to_100(`This function has a clear and inspiring vision`),
    `This function has credibility and a well-defined role across the organization` = rescale_to_100(`This function has credibility and a well-defined role across the organization`),
    `This function has the leadership team to deliver effectively to achieve its potential` = rescale_to_100(`This function has the leadership team to deliver effectively to achieve its potential`),
    `This function has a direct impact and contribution to Clearwater achieving its goals around growth and innovation.` = rescale_to_100(`This function has a direct impact and contribution to Clearwater achieving its goals around growth and innovation.`),
    `This function has the team and resources to achieve its goals` = rescale_to_100(`This function has the team and resources to achieve its goals`)
  )
...
group_averages <- df_final %>%
  group_by(SheetName) %>%
  summarise(
    `Clear and Inspiring Vision` = mean(`This function has a clear and inspiring vision`, na.rm = TRUE),
    `Credibility and Well-Defined Role` = mean(`This function has credibility and a well-defined role across the organization`, na.rm = TRUE),
    `Leadership Team to Deliver Effectively` = mean(`This function has the leadership team to deliver effectively to achieve its potential`, na.rm = TRUE),
    `Impact and Contribution to Growth and Innovation` = mean(`This function has a direct impact and contribution to Clearwater achieving its goals around growth and innovation.`, na.rm = TRUE),
    `Team and Resources to Achieve Goals` = mean(`This function has the team and resources to achieve its goals`, na.rm = TRUE)
  ) %>%
...
process_data <- function(data, sheet_name, segment = FALSE) {
  ...
  data <- data %>%
    mutate(across(starts_with("This function has"), recode_responses)) %>%
    mutate(across(starts_with("This function has"), rescale_to_100)) %>%
  ...
  if (segment) {
    ...
    grouped_data <- data %>%
      ...
      summarise(across(starts_with("This function has"), mean, na.rm = TRUE), .groups = 'drop')
    overall_data <- data %>%
      group_by(SheetName) %>%
      summarise(across(starts_with("This function has"), mean, na.rm = TRUE), .groups = 'drop')
...
combined_data_long <- combined_data %>%
  pivot_longer(cols = starts_with("This function has"), names_to = "Question", values_to = "Score")
...
# Create plots for the function and leader column
create_plots(combined_data_long, "Select the option (function and leader) you work with:", "Function and Leader")

# Create plots for the category column
create_plots(combined_data_long, "Which category do you work within?", "Category")
...
# Create individual plots for each question, grouped by the function and leader column
create_individual_plots(combined_data_long, "Select the option (function and leader) you work with:", "Function and Leader")

# Create individual plots for each question, grouped by the category column
create_individual_plots(combined_data_long, "Which category do you work within?", "Category")
