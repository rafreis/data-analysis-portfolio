...
setwd("C:/Users/rafre/Dropbox/Fiverr/Trabalhos/2024/trackrecord/Fourth Wave/Employee_Survey")

# Authenticate with Google Sheets
#gs4_auth()  # Uncomment this line if authentication is needed

# Specify the Google Sheet URL
sheet_url <- "https://docs.google.com/spreadsheets/d/1v3oq6acDRKuNI7oh1A1gmXnVwPgC7R4dGVfIMJT5e9U/edit?gid=1880195926#gid=1880195926"
...
group_averages <- df_final %>%
  ...
  summarise(
    ...
    `Impact on Growth and Innovation` = mean(`This function has a direct impact and contribution to Clearwater achieving its goals around growth and innovation.`, na.rm = TRUE),
...
group_averages_byorg <- df_final %>%
  group_by(Group = `Select the division/ leader that you work under`, Group) %>%
  summarise(
    `Clear and Inspiring Vision` = mean(`This function has a clear and inspiring vision`, na.rm = TRUE),
...
group_averages_long <- group_averages_byorg %>%
  pivot_longer(
    cols = -c(Group, Group, Sample_Size),
    names_to = "Metric", 
    values_to = "Value"
  ) %>%
  filter(!is.na(Group))  # Exclude records where Group_Type is NA

# Function to create and save plots
create_plot <- function(metric_name) {
  p <- ggplot(group_averages_long %>% filter(Metric == metric_name, !is.na(Value)), aes(x = Group, y = round(Value, 0), fill = Group)) +
