...
calculate_stats <- function(data, variables) {
  ...
  results <- data.frame(
    ...
    KS_D = numeric(),  # New column for KS statistic
    KS_p_value = numeric(),  # New column for KS p-value
  ...
  for (var in variables) {
    if (var %in% names(data)) {
      ...
      # Perform Kolmogorov-Smirnov test, if sufficient data points
      if (n_val >= 3) {  # KS test requires at least 3 data points
        ks_test <- ks.test(na.omit(data[[var]]), "pnorm", mean = mean(data[[var]], na.rm = TRUE), sd = sd(data[[var]], na.rm = TRUE))
      ...
      results <- rbind(results, data.frame(
        ...
        KS_D = ks_stat,  # Include KS statistic
        KS_p_value = ks_p_value,  # Include KS p-value
        N = n_val  # Include the sample size
      ))
    } else {
...
save_apa_formatted_excel(data_list, "APA_Formatted_Tables_withN.xlsx")