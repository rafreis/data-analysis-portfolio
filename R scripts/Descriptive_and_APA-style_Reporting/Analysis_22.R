...
# Generate a dataframe with mean scores for each farm and each population
mean_scores_df <- data %>%
  group_by(Farm, Population) %>%
  summarise(across(dependent_vars, mean, na.rm = TRUE))

mean_scores_df_wide <- mean_scores_df %>%
  pivot_wider(names_from = Population, values_from = dependent_vars)

# Summarize across all farms
mean_scores_all_farms <- data %>%
  group_by(Population) %>%
  summarise(across(dependent_vars, mean, na.rm = TRUE))

# Reshape to wide format like mean_scores_df_wide
...
mean_scores_df_wide <- rbind(mean_scores_df_wide, mean_scores_all_farms_wide)
