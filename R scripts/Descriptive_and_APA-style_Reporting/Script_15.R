...
library(openxlsx)
df <- read.xlsx("C:/Users/rafre/Dropbox/Fiverr/Trabalhos/2024/chippie77/Data2.xlsx")
...
select_adele_cases <- function(data, patient_id_col, adele_col) {
  data %>%
    # Ensure data is sorted by patient and within patient by date or interview
    arrange(!!sym(patient_id_col), Date.of.interview) %>%
    # Group by patient
    group_by(!!sym(patient_id_col)) %>%
    # Add a row number to identify the first "yes"
    mutate(row_num = row_number(),
           first_yes_row = which(!!sym(adele_col) == "yes")[1]) %>%
    # Select rows where "Adele User = Yes" or the row immediately before the first "yes"
    filter((row_num == first_yes_row & !!sym(adele_col) == "yes") |
             (row_num == first_yes_row - 1 & !!sym(adele_col) == "no")) %>%
    # Remove helper columns
    select(-c(row_num, first_yes_row)) %>%
    # Ungroup data
    ungroup()
}


df <- select_adele_cases(df, "Patient", "ADELE.user.Yes_No")
...
calculate_stats <- function(data, variables) {
  ...
  for (var in variables) {
    if (var %in% names(data)) {
      ...
      results <- rbind(results, c(var, skew, kurt, shapiro_test$statistic, shapiro_test$p.value))
    } else {
      warning(paste("Variable", var, "not found in the data. Skipping."))
    }
  }
  
  colnames(results) <- c("Variable", "Skewness", "Kurtosis", "Shapiro_Wilk_F", "Shapiro_Wilk_p_value")
  return(results)
}
...
df_descriptive_stats_bygroup_dipa <- calculate_descriptive_stats_bygroups(df_dipa, scales_dipa, "ADELE.user.Yes_No")
df_descriptive_stats_bygroup_diga <- calculate_descriptive_stats_bygroups(df_diga, scales_diga, "ADELE.user.Yes_No")
...
create_mean_sd_plot <- function(data, variables, factor_column) {
  ...
  p <- ggplot(long_data, aes(x = !!sym(factor_column), y = Mean)) +
    ...
    labs(
      title = "Mean and Standard Deviation by Adele Use", 
      x = "Adele Use",  # Rename the x-axis title
      y = "Value",
      color = "Adele Use"  # Rename the legend title
    ) +
    scale_color_discrete(name = "Adele Use")  # Rename the legend title
...
factor_var <- "ADELE.user.Yes_No"
plot <- create_mean_sd_plot(df_diga, scales_diga, factor_var)
print(plot)
ggsave("mean_sd_plot_diga.png", plot = plot, width = 12, height = 8)

plot <- create_mean_sd_plot(df_dipa, scales_dipa, factor_var)
print(plot)
...
create_mean_ci_plot <- function(data, variables, factor_column, legend_title) {
  # Create a long format of the data for ggplot
  long_data <- data %>%
    pivot_longer(cols = all_of(variables), names_to = "Variable", values_to = "Value") %>%
    group_by(!!sym(factor_column), Variable) %>%
  ...
  p <- ggplot(long_data, aes(x = !!sym(factor_column), y = Mean)) +
    ...
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) +
    labs(
      title = "Mean and 95% Confidence Interval by Adele Use", 
      x = "Adele Use",  # Rename the x-axis title
      y = "Value",
      color = legend_title  # Set the legend title
    ) +
    scale_color_discrete(name = legend_title)  # Ensure this matches your intended legend title
  
  # Print the plot
  return(p)
}

plot <- create_mean_ci_plot(df_diga, scales_diga, factor_var, "Adele Use")
print(plot)
ggsave("mean_ci_plot_diga.png", plot = plot, width = 12, height = 8)

plot <- create_mean_ci_plot(df_dipa, scales_dipa, factor_var, "Adele Use")
print(plot)
...
df_dipa <- df_dipa %>%
  mutate(
    development_2 = if_else(Patient == 98, NA_real_, development_2)
  )
...
fit_lmm_and_format <- function(data, within_subject_var, random_effects, response_vars, save_plots = FALSE) {
  lmm_results_list <- list()
  
  for (response_var in response_vars) {
    # Construct the formula dynamically without the between-subjects factor
    formula <- as.formula(paste(response_var, "~", within_subject_var, "+", random_effects))
    ...
    lmm_results <- tidy(lmer_model, effects = "fixed") %>%
      mutate(ResponseVariable = response_var)  # Add the response variable name
...
lmm_results_list_dipa <- fit_lmm_and_format(
  data = df_dipa,
  within_subject_var = "ADELE_user_Yes_No",
...
# Adding an argument for additional fixed effects
fit_lmm_and_format_control <- function(data, within_subject_var, random_effects, response_vars, additional_fixed_effects = NULL, save_plots = FALSE) {
  lmm_results_list <- list()
  
  for (response_var in response_vars) {
    # Start constructing the formula
    formula_string <- paste(response_var, "~", within_subject_var)
...
# Example usage:
# Now you can include additional fixed effects like this:
