...
fit_gamm_and_format <- function(data, indicators, save_plots = FALSE) {
  gamm_results_list <- list()
  gamm_models_list <- list()
  ...
  for (indicator in indicators) {
    ...
    # Fit the GAMM model with autoregressive correlation structure
    gamm_model <- gamm(
      Value ~ Location + s(DateTime),  # Smooth term for time dependence
      random = list(Individual = ~1),  # Random effect for individuals
      correlation = corAR1(form = ~ DateTime | Individual),  # Autoregressive structure
      data = data_subset
    )
    
    # Store the model in the list
    gamm_models_list[[indicator]] <- gamm_model$lme  # Extract the LME part of GAMM
    
    # Extract fixed effects results (includes p-values)
    fixed_effects_results <- tidy(gamm_model$lme, effects = "fixed") %>%
      mutate(Indicator = indicator)
    
    # Extract random effects variance
    random_effects_results <- as.data.frame(VarCorr(gamm_model$lme))
    
    # Store results
    gamm_results_list[[indicator]] <- list(
      fixed = fixed_effects_results,
      random = random_effects_results
    )
    
    # Print model summary
    print(summary(gamm_model$lme))
    ...
    if (save_plots) {
      ...
      plot(residuals(gamm_model$lme) ~ fitted(gamm_model$lme), main = "Residuals vs Fitted", xlab = "Fitted values", ylab = "Residuals")
      dev.off()
      
      jpeg(paste0(plot_name_prefix, "_QQ_Plot.jpeg"))
      qqnorm(residuals(gamm_model$lme), main = "Q-Q Plot")
      qqline(residuals(gamm_model$lme))
      dev.off()
    }
  }
  
  # Combine fixed effects results into a tidy dataframe
  df_gamm_results_fixed <- bind_rows(lapply(gamm_results_list, `[[`, "fixed"))
  
  return(list(results = df_gamm_results_fixed, models = gamm_models_list))  # Return results + models
}
...
gamm_results <- fit_gamm_and_format(df_airquality_long_clean, response_vars, save_plots = TRUE)
